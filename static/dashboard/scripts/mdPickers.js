...
function DatePickerCtrl($scope, $mdDialog, currentDate,minDate, $mdMedia, $timeout) {
    var self = this;
    this.currentDate = currentDate;
    this.currentMoment = moment(self.currentDate);
...
module.provider("$mdpDatePicker", function() {
    ...
    this.$get = ["$mdDialog", function($mdDialog) {
        var datePicker = function(targetEvent, currentDate,minDate) {
            if(!angular.isDate(currentDate)) currentDate = Date.now();
            minDate=Date.parse(minDate);
            return $mdDialog.show({
                controller:  ['$scope', '$mdDialog', 'currentDate','minDate', '$mdMedia', '$timeout', DatePickerCtrl],
                ...
                template: '<md-dialog aria-label="" class="mdp-datepicker" ng-class="{ \'portrait\': !$mdMedia(\'gt-xs\') }">' +
                            '<md-dialog-content layout="row" layout-wrap>' +
                                ...
                                '<div>' + 
                                    ...
                                    '</div>' +
                                    '<mdp-calendar ng-if="!datepicker.selectingYear" class="mdp-animation-zoom" date="datepicker.currentMoment" minDate="datepicker.minDate"></mdp-calendar>' +
                                    '<md-dialog-actions layout-align="end center" layout="row">' +
...
function CalendarCtrl($scope) {
    ...
    this.isDisabledMonth = function() {
         var testMoment = moment(this.currentMoment);
        if (this.minDate){
            if (this.minDate.isBefore(testMoment, 'month')){
                return false;
            }
            return true;
        }
    ...
    this.isDisabled = function(day){
        ...
        testMoment.date(day);
        if (this.minDate){
            if (this.minDate.isBefore(testMoment, 'day')){
                return false;
            }
            return true;
        }
    ...
    return {
        ...
        template: '<div class="mdp-calendar">' +
                    '<div layout="row" layout-align="space-between center">' +
                        '<md-button aria-label="previous month" class="md-icon-button" ng-disabled="calendar.isDisabledMonth()" ng-click="calendar.prevMonth()"><md-icon md-font-set="material-icons"> chevron_left </md-icon></md-button>' +
...
/*module.directive("mdpDatePicker", ["$mdpDatePicker", "$timeout", function($mdpDatePicker, $timeout) {
    return  {
        ...
        link: function(scope, element, attrs, ngModel) {
            if ('undefined' !== typeof attrs.type && 'text' === attrs.type && ngModel) {
                angular.element(element).on("click", function(ev) {
                  		ev.preventDefault();
                      $mdpDatePicker(ev, ngModel.$modelValue,attrs.minDate).then(function(selectedDate) {
                          $timeout(function() { 
                            	ngModel.$setViewValue(moment(selectedDate).format("DD-MM-YYYY")); 
                            	ngModel.$render(); 
                                element.value(moment(selectedDate).format("DD-MM-YYYY"));
                          });
                      });
                });
            }

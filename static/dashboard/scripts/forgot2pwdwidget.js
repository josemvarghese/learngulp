...
function _MakePWDWidget()
{
	...
	code += "<input stopccp type='password' ng-focus='pwderr=false' ng-blur='pwderr=true' class=' nav-width' placeholder='New Password'  name='"+pwdname+"' id='"+this.pwdfieldid+"' ng-model='pwd' minlength='8' required equals='{{cnfpwd}}' pwdpolicy maxlength='32'>";

	this.pwdtxtfield=pwdname+"_text";

	this.pwdtxtfieldid = this.pwdtxtfield+"_id";

	//code += "<input type='text' class='pwdfield nav-width' name='"+this.pwdtxtfield+"' id='"+this.pwdtxtfieldid+"' style='display: none;'  ng-model='pwd' required equals='{{cnfpwd}}' >";
	...
	code += "<div class='pwdstrength-fgt' id='"+this.pwdstrengthdiv+"'>";
...
function passwordStrength()
{
	...
	{
		strengthdiv.style.width=(score+1)*35+'px';
	}
